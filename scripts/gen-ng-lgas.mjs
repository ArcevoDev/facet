// Generator: builds the complete Nigeria LGA dataset (774 LGAs across
// 36 states + FCT) and prints the DEFAULT_LOCALITIES.NG block for
// location-data.ts. Run: node scripts/gen-ng-lgas.mjs
// Source: official INEC list of Local Government Areas of Nigeria.
...
// Transform to the Locality[] shape with kebab ids.
const kebab = (s) => s.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");

let total = 0;
const lines = ["  NG: {"];
for (const [state, lgAs] of Object.entries(NIGERIA_LGAS)) {
  lines.push(`    ${JSON.stringify(state)}: [`);
  for (const name of lgAs) {
    // State-scoped id: avoids collisions between states (e.g. "Nasarawa"
    // in Kano vs Nasarawa state, "Surulere" in Lagos vs Oyo).
    lines.push(
      `      { id: ${JSON.stringify(`${state}-${kebab(name)}`)}, name: ${JSON.stringify(name)} },`,
    );
    total++;
  }
  lines.push("    ],");
...
console.log(`// Generated: ${total} LGAs across ${Object.keys(NIGERIA_LGAS).length} states + FCT`);
